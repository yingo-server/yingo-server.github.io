...
export class Game {
    constructor(canvas, mobileControlsEl) {
        ...
        this.slideStartSent = false;   // 防止重复发送 start
    ...
    setupInputListeners() {
        ...
        this.setupActionButton('btn-slide', (pressed) => {
            ...
            if (pressed) {
                ...
                this.slideStartSent = false;   // 重置，允许新的滑铲请求
            }
            // 松开时不发送任何动作（滑铲一旦开始就不受松开影响）
        });
        // 技能按钮
    ...
    animate() {
        ...
        // 长按滑铲检测：按住超过200ms且未发送过，且不在滑铲中，则发送 start
        if (this.inputRaw.slide && !this.slideStartSent && this.player && !this.player.isSliding) {
