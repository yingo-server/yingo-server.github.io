(function() {
    ...
    // 存储认证信息（30天）
    function saveAuth(token, userid, dfid) {
        const expire = Date.now() + LOGIN_EXPIRE_DAYS * 24 * 60 * 60 * 1000;
        const auth = { token, userid, dfid, expire };
    ...
    async function request(path, params = {}) {
        ...
        if (TOKEN && USERID && DFID) {
            params.cookie = `token=${TOKEN};userid=${USERID};dfid=${DFID}`;
        }
        ...
        try {
            ...
            if (json.status !== 1 && json.error_code !== 0) {
                // 特殊处理 20010 不抛异常，留给调用方判断
                if (json.error_code === 20010) {
                    console.warn('[request] error_code=20010', json);
                    return json;
                }
                throw new Error(json.message || `接口错误 code=${json.error_code}`);
            }
            return json;
        } catch (e) {
    ...
    async function ensureDfid() {
        try {
            let res = await request('/register/dev');
            DFID = res.data?.dfid;
            if (TOKEN && USERID) saveAuth(TOKEN, USERID, DFID);
            else localStorage.setItem('dfid', DFID);
        } catch(e) { toast('获取设备标识失败', true); }
    }

    // ---------- 登录相关 ----------
    async function sendCode(mobile) {
        await request('/captcha/sent', { mobile });
        toast('验证码已发送');
    }
    ...
    async function loginWithCode(mobile, code) {
        ...
        if (res.data?.token) {
            saveAuth(res.data.token, res.data.userid, DFID);
            toast('登录成功');
    ...
    async function startQR() {
        ...
        if (!key) { toast('获取二维码key失败', true); return; }
        let qrRes = await request('/login/qr/create', { key, qrimg: 1 });
        let imgData = qrRes.data?.qrcode_img;
        if (!imgData) { toast('二维码生成失败', true); return; }
        let qrBox = document.querySelector('#loginQrBox');
        if (qrBox) qrBox.innerHTML = `<img src="${imgData}" style="width:100%;height:auto;">`;
        ...
        qrTimer = setInterval(async () => {
            ...
            if (status === 4) {
                ...
                if (check.data?.token) {
                    saveAuth(check.data.token, check.data.userid, DFID);
                    toast('扫码登录成功');
                    loginOverlay.classList.remove('show');
                    loadMyPage();
                    loadHome();
    ...
    // ---------- 核心：获取歌曲 URL（带 album_id 和 album_audio_id） ----------
    async function getSongUrl(song) {
        const { hash, album_audio_id, album_id } = song;
        if (!hash) return null;

        // 尝试各种 quality，同时传递 album_id 和 album_audio_id
        ...
        let url = extractUrlFromResponse(newRes);
        if (url) return url;

        // 终极正则：从所有响应文本中提取 mp3/flac 链接，选最短的
        let allResponses = [];
        // 重新请求一遍并收集原始字符串（因为上面的 request 已经解析为json）
        // 为了获取原始响应，这里直接 fetch 并获取 text
        for (let q of qualities) {
            let params = { hash, quality: q };
            if (album_audio_id) params.album_audio_id = album_audio_id;
            if (album_id) params.album_id = album_id;
            ...
            params.timestamp = Date.now();
            if (TOKEN && USERID && DFID) params.cookie = `token=${TOKEN};userid=${USERID};dfid=${DFID}`;
        ...
        let newParams = { hash, timestamp: Date.now() };
        if (TOKEN && USERID && DFID) newParams.cookie = `token=${TOKEN};userid=${USERID};dfid=${DFID}`;
    ...
    function extractUrlFromResponse(res) {
        if (res.error_code === 20010) return null; // 明确错误不提取
        if (res.data?.url && Array.isArray(res.data.url) && res.data.url.length) return res.data.url[0];
        if (res.data?.backupUrl && Array.isArray(res.data.backupUrl) && res.data.backupUrl.length) return res.data.backupUrl[0];
        if (Array.isArray(res.data) && res.data[0]?.info?.tracker_url?.length) return res.data[0].info.tracker_url[0];
        if (res.data?.info?.tracker_url?.length) return res.data.info.tracker_url[0];
        return null;
    }
    ...
    function buildLoginDialog() {
        ...
        updateSlider();
        document.getElementById('loginSendBtn').onclick = async () => {
    ...
    async function init() {
        if (loadAuth()) {
            if (!DFID) await ensureDfid();
            else {
                if (TOKEN && USERID && DFID) saveAuth(TOKEN, USERID, DFID);
            }
        } else {
            await ensureDfid();
        }
