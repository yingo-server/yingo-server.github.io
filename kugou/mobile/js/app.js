/* =====================================================
   锐角音乐 · 播放器核心
   版本: 0.05.001
   更新: 非VIP智能选择最长可播放片段（试听/高潮），并检测提前结束
===================================================== */
(function() {
    // ---------- 配置 ----------
    ...
    function addAccount(token, userid, nickname, pic) {
        let accounts = getAccounts();
        const exist = accounts.find(a => a.userid == userid);
        if (exist) {
            exist.token = token;
            exist.nickname = nickname || exist.nickname;
            exist.pic = pic || exist.pic;
        } else {
            accounts.push({ token, userid, nickname, pic });
        }
    ...
    function getCurrentAccount() {
        ...
        return accounts.find(a => a.userid == uid) || null;
    }
    ...
    function switchToAccount(userid) {
        ...
        const switchOverlay = document.getElementById('switchAccountOverlay');
        if (switchOverlay) switchOverlay.classList.remove('show');
        isVip = false;
        fetchAndCacheVipStatus().then(() => {
            loadMyPage();
            loadHome();
        });
        toast('切换账号成功');
    ...
    function clearUserCache(userid) {
        ...
        const prefixes = [
            `user_detail_${userid}`,
            `user_vip_detail_${userid}`,
            `user_playlist_${userid}`,
            `playlist_tracks_${userid}_`,
            `playlist_total_${userid}_`
        ];
        prefixes.forEach(prefix => {
            CacheManager.clearPrefix(prefix, true);
            CacheManager.clearPrefix(prefix, false);
        });
        isVip = false;
    }
    ...
    function bindAudioEvents() {
        ...
        audio.onended = () => {
            // 片段播放结束检测
            if (isClimax || isTrial) {
                // 如果预期结束时间大于0，且当前播放时间小于预期结束时间-5秒，视为提前截断
                if (expectedEndMs > 0 && (audio.currentTime < (expectedEndMs / 1000) - 5)) {
                    toast('片段播放结束，开通VIP可听完整版', false);
                    audio.pause();
                    // 可选：自动下一首，由调用方决定。这里我们不自动下一首，让用户手动操作。
                    return;
                }
            }
            // 正常结束
    ...
    function updateProgressAndLyrics() {
        ...
        let range = document.querySelector('#playerProgressRange');
        if (range) {
            let percent = (cur / dur) * 100;
            let val = isNaN(percent) ? 0 : percent / 100;
            range.value = val;
            let fillPercent = (val / range.max) * 100;
            range.style.background = `linear-gradient(to right, var(--accent) ${fillPercent}%, var(--border-subtle) ${fillPercent}%)`;
        }
    ...
    function initProgressBar() {
        const range = document.querySelector('#playerProgressRange');
        if (!range) return;
        range.addEventListener('input', function() {
            if (audio.duration) {
                const val = parseFloat(this.value);
                audio.currentTime = val * audio.duration;
                let fillPercent = (val / this.max) * 100;
                this.style.background = `linear-gradient(to right, var(--accent) ${fillPercent}%, var(--border-subtle) ${fillPercent}%)`;
            }
        });
    }
    ...
    // ---------- 核心请求 ----------
    async function request(path, params = {}, skipCache = false, cacheTtl = null, cacheKey = null, useLocalStorage = false) {
        if (!DFID) {
            const ok = await ensureDfid();
            if (!ok) throw new Error('无法获取设备标识');
        }
        if (!skipCache && cacheKey) {
            const cached = CacheManager.get(cacheKey, useLocalStorage);
        ...
        try {
            ...
            if (json.status === 2 && path !== '/song/url' && path !== '/song/url/new') {
            ...
            if (!skipCache && cacheKey && isSuccess && cacheTtl) {
                CacheManager.set(cacheKey, json, cacheTtl, useLocalStorage);
            }
            return json;
        } catch (e) {
    ...
    // ---------- VIP状态获取 ----------
    async function fetchAndCacheVipStatus() {
        if (!TOKEN) return false;
        const vipKey = `user_vip_detail_${USERID}`;
        let vipRes = CacheManager.get(vipKey, true);
        if (!vipRes) {
            vipRes = await request('/user/vip/detail', {}, false, 12*3600, vipKey, true);
        }
        if (vipRes && vipRes.data) {
            isVip = !!vipRes.data.is_vip;
    ...
    async function handleLoginSuccess(token, userid, nickname, pic) {
        addAccount(token, userid, nickname, pic);
        const oldUserid = USERID;
        if (oldUserid && oldUserid != userid) {
            clearUserCache(oldUserid);
        }
    ...
    async function loginWithCode(mobile, code) {
        ...
        if (res?.data?.token) {
            ...
            await handleLoginSuccess(token, userid, nickname, pic);
            return true;
        }
        ...
        qrTimer = setInterval(async () => {
            ...
            if (status === 4) {
                ...
                if (check?.data?.token) {
                    ...
                    const userid = check.data.userid;
                    const nickname = check.data.nickname || '';
                    const pic = check.data.pic || '';
                    await handleLoginSuccess(token, userid, nickname, pic);
                }
            } else if (status === 0) {
                clearInterval(qrTimer);
    ...
    function showSwitchAccountDialog() {
        ...
        let overlay = document.getElementById('switchAccountOverlay');
        if (!overlay) {
            overlay = document.createElement('div');
            overlay.id = 'switchAccountOverlay';
            overlay.className = 'switch-account-overlay';
            document.body.appendChild(overlay);
        }
        const currentUid = getCurrentUserid();
        let html = `
            <div class="switch-account-card">
                <div class="card-header">
                    <h3>切换账号</h3>
                    <button class="close-btn" id="closeSwitchBtn">&times;</button>
                </div>
                <div class="account-list">
        `;
        accounts.forEach(acc => {
            const isCurrent = acc.userid == currentUid;
            html += `
                <div class="account-item ${isCurrent ? 'current' : ''}" data-userid="${acc.userid}">
                    <div class="avatar">${acc.pic ? `<img src="${imgUrl(acc.pic, 48)}">` : '---'}</div>
                    <div class="info">
                        <div class="name">${escapeHtml(acc.nickname || '用户')}</div>
                        <div class="id">ID: ${acc.userid}</div>
                    </div>
        ...
        html += `
                ...
                <div class="actions">
                    <button id="addAccountBtn">+ 添加新账号</button>
                </div>
            </div>
        `;
        ...
        document.getElementById('closeSwitchBtn').onclick = () => {
            overlay.classList.remove('show');
        };
        overlay.onclick = (e) => {
    ...
    // ---------- URL 提取与片段选择 ----------
    function isEncryptedUrl(url) {
        if (!url) return true;
        return url.includes('.mgg') || url.includes('.mflac');
    }

    function extractCandidatesFromNewResponse(res) {
        let candidates = [];
        if (Array.isArray(res.data) && res.data.length > 0) {
            let info = res.data[0].info;
            if (!info) return candidates;
            // 试听片段 (tracker_url)
            if (info.tracker_url && info.tracker_url.length > 0) {
                let url = info.tracker_url[0];
                if (!isEncryptedUrl(url)) {
                    let endMs = info.hash_offset?.end_ms || 0;
                    candidates.push({ url, endMs, isClimax: false, isTrial: true });
                }
            }
            // 高潮片段 (climax_info.url)
            if (info.climax_info && info.climax_info.url && info.climax_info.url.length > 0) {
                let url = info.climax_info.url[0];
                if (!isEncryptedUrl(url)) {
                    let endMs = info.climax_info.hash_offset?.end_ms || 0;
                    candidates.push({ url, endMs, isClimax: true, isTrial: false });
                }
            }
        }
    ...
    async function getSongUrl(song) {
        ...
        if (cached) {
            if (typeof cached === 'string') {
                ...
                isClimax = cached.isClimax || false;
                isTrial = cached.isTrial || false;
                expectedEndMs = cached.expectedEndMs || 0;
                return cached.url;
            }
        }
        ...
        // VIP用户：优先旧版320k
        if (isVip) {
            toast('VIP用户，获取320k链接...');
            let params = { hash, quality: '320' };
            if (album_audio_id) params.album_audio_id = album_audio_id;
            if (album_id) params.album_id = album_id;
            let res = await request('/song/url', params);
            let url = null;
            if (res.url && Array.isArray(res.url) && res.url.length > 0) {
                url = res.url[0];
            ...
            if (url && !isEncryptedUrl(url)) {
                ...
                isTrial = false;
                expectedEndMs = 0;
                CacheManager.set(cacheKey, { url, isClimax: false, isTrial: false, expectedEndMs: 0 }, 86400);
                return url;
            } else {
                toast('VIP链接获取失败或加密', true);
                return null;
            }
        }
        ...
        if (album_audio_id) params.album_audio_id = album_audio_id;
        if (album_id) params.album_id = album_id;
        let oldRes = await request('/song/url', params);
        let oldUrl = null;
        if (oldRes.url && Array.isArray(oldRes.url) && oldRes.url.length > 0) {
            oldUrl = oldRes.url[0];
        } else if (oldRes.data && oldRes.data.url) {
            oldUrl = oldRes.data.url;
        }
        if (oldUrl && !isEncryptedUrl(oldUrl)) {
            toast('获取成功');
            isClimax = false;
            isTrial = false;
            expectedEndMs = 0;
            CacheManager.set(cacheKey, { url: oldUrl, isClimax: false, isTrial: false, expectedEndMs: 0 }, 86400);
            return oldUrl;
        }

        // 2. 旧版失败或加密，尝试新版
        toast('尝试试听/高潮片段...');
        let newRes = await request('/song/url/new', { hash });
        let candidates = extractCandidatesFromNewResponse(newRes);
        if (candidates.length > 0) {
            let best = candidates[0];
            if (best.url && !isEncryptedUrl(best.url)) {
                isClimax = best.isClimax || false;
                isTrial = best.isTrial || false;
                expectedEndMs = best.endMs || 0;
                let msg = isClimax ? '使用高潮片段' : '使用试听片段';
                if (expectedEndMs > 0) {
                    msg += ` (预计 ${Math.floor(expectedEndMs/1000)} 秒)`;
                }
                toast(msg);
                CacheManager.set(cacheKey, { url: best.url, isClimax, isTrial, expectedEndMs }, 86400);
                return best.url;
            }
        }

        // 所有方式失败
        toast('无法获取可播放链接', true);
        return null;
    }
    ...
    function tryRestorePlaylistFromCache() {
        ...
        for (let page = 1; page <= currentPlaylistMaxPage; page++) {
            const cacheKey = `playlist_tracks_${USERID}_${currentPlaylistId}_page${page}`;
    ...
    async function loadOnePlaylistPage(page) {
        ...
        if (!res) {
            res = await request('/playlist/track/all/new', { listid: currentPlaylistId, page: page, pagesize: 50 }, false, 86400*30, cacheKey, true);
    ...
    async function openPlaylist(pl) {
        ...
        const totalCacheKey = `playlist_total_${USERID}_${currentPlaylistId}`;
        const cachedTotal = CacheManager.get(totalCacheKey, true);
        if (cachedTotal !== null && cachedTotal !== currentPlaylistTotal) {
            CacheManager.clearPlaylistCache(currentPlaylistId);
        }
        CacheManager.set(totalCacheKey, currentPlaylistTotal, 86400*30, true);
        ...
        appDiv.innerHTML = `
            ...
            <div class="player-full" id="fullPlayer">
                ...
                <div class="player-progress-row">
                    ...
                    <input type="range" id="playerProgressRange" class="player-progress-range" min="0" max="1" step="any" value="0">
    ...
    async function loadMyPage() {
        ...
        try {
            ...
            if (!userRes) {
                userRes = await request('/user/detail', {}, false, 12*3600, userKey, true);
            }
            const vipKey = `user_vip_detail_${USERID}`;
            let vipRes = CacheManager.get(vipKey, true);
            if (!vipRes) {
            ...
            if (!playlistRes) {
                playlistRes = await request('/user/playlist', {}, false, 12*3600, playlistKey, true);
            }
            ...
            document.getElementById('myHeader').onclick = showSwitchAccountDialog;
