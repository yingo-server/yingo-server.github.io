(function() {
    ...
    const CacheManager = {
        ...
        set(key, data, ttlSeconds, useLocalStorage = false) {
    ...
    async function getSongUrl(song) {
        ...
        if (url) {
            toast('成功获取 320k 链接');
            CacheManager.set(cacheKey, url, 600);
            return url;
        } else {
        ...
        if (matches && matches.length) {
            ...
            toast('正则匹配成功');
            CacheManager.set(cacheKey, matches[0], 600);
            return matches[0];
        }
        toast('所有方式均失败', true);
        return null;
    ...
    // 预加载更多歌单歌曲（播放时自动加载）
    async function preloadMoreIfNeeded() {
        if (!currentPlaylistId) return;
        if (playlistLoading) return;
        if (currentPlaylistPage <= 0) return;
        // 如果当前播放索引距离列表末尾小于5，则预加载下一页
        if (currentIndex >= currentPlaylist.length - 5) {
    ...
    async function loadHome() {
        try {
            ...
            if (!daily) {
                daily = await request('/everyday/recommend', { platform: 'ios' }, false, getSecondsUntilMidnight(), dailyKey);
            }
            // 猜你喜欢（缓存10分钟）
    ...
    async function openPlaylist(pl) {
        ...
        if (listEl) {
            listEl.onscroll = () => {
                if (listEl.scrollTop + listEl.clientHeight >= listEl.scrollHeight - 80 && !playlistLoading && currentPlaylistPage > 0) {
                    loadPlaylistPage();
                }
            };
        }
    ...
    async function loadPlaylistPage() {
        if (playlistLoading || currentPlaylistPage < 1) return;
        playlistLoading = true;
        let loadHint = document.getElementById('plLoadMore');
        if (loadHint) loadHint.textContent = '加载中...';
        try {
            const cacheKey = `playlist_tracks_${currentPlaylistId}_page${currentPlaylistPage}`;
            let res = CacheManager.get(cacheKey);
            if (!res) {
                res = await request('/playlist/track/all/new', { listid: currentPlaylistId, page: currentPlaylistPage, pagesize: 50 }, false, 600, cacheKey);
            }
        ...
        if (loadHint) loadHint.textContent = currentPlaylistPage <= 0 ? '已加载全部' : '上滑加载更多';
