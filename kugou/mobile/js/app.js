(function() {
    ...
    // 环境变量：0 = 当前模式（VIP判断），1 = 未来概念版模式（待适配）
    const USE_CONCEPT_API = 0;
    ...
    // 试听标志
    let isTrial = false;      // 当前播放是否为试听片段
    let trialTimer = null;    // 用于提示的计时器
    ...
    function bindAudioEvents() {
        ...
        audio.ontimeupdate = () => {
            ...
            // 试听时长提醒
            if (isTrial && audio.currentTime >= 58 && audio.currentTime < 60) {
                if (!trialTimer) {
                    trialTimer = setTimeout(() => {
                        toast('试听片段结束，开通 VIP 可听完整版', false);
                        audio.pause();
                        trialTimer = null;
                    }, 100);
                }
            } else if (audio.currentTime < 58) {
                if (trialTimer) {
    ...
    function updateProgressAndLyrics() {
        ...
        if (fill) fill.style.width = (isNaN(percent) ? 0 : percent) + '%';
    ...
    async function getSongUrl(song) {
        ...
        if (USE_CONCEPT_API === 0) {
            if (isVip) {
                // VIP用户：直接使用旧版320k，不降级
                toast('VIP账号，获取320k链接...');
                ...
                url = extractPlayableUrl(res);
                usedTrial = false;
                if (!url) {
                    toast('VIP链接获取失败', true);
                    return null;
                }
            } else {
                // 非VIP用户：先尝试旧版320k，失败再尝试新版试听
                toast('尝试获取320k链接...');
                let params = { hash, quality: '320' };
                if (album_audio_id) params.album_audio_id = album_audio_id;
                if (album_id) params.album_id = album_id;
                ...
                if (url) {
                    ...
                    toast('320k失败，尝试试听链接...', true);
                    let res2 = await request('/song/url/new', { hash });
                    url = extractPlayableUrl(res2);
                    usedTrial = true;
                    if (url) {
                        toast('试听链接获取成功（仅59秒）');
                    } else {
            ...
            toast('概念版模式暂未适配', true);
            return null;
        }

        if (url) {
            // 缓存URL及是否为试听
            CacheManager.set(cacheKey, { url, isTrial: usedTrial }, 86400);
            isTrial = usedTrial;
            return url;
        }
        toast('无法获取播放链接', true);
    ...
    function initProgressBarEvents() {
        ...
        bar.addEventListener('click', (e) => {
            ...
            const newTime = audio.duration * Math.min(1, Math.max(0, percent));
        ...
        bar.addEventListener('mousedown', (e) => {
            ...
            if (fill) fill.style.width = dragPercent * 100 + '%';
        ...
        bar.addEventListener('touchstart', (e) => {
            ...
            dragPercent = (touch.clientX - rect.left) / rect.width;
            dragPercent = Math.min(1, Math.max(0, dragPercent));
            const fill = document.querySelector('#playerProgressFill');
            if (fill) fill.style.width = dragPercent * 100 + '%';
    ...
    function onMouseUp() {
        if (!isDragging) return;
        isDragging = false;
        audio.currentTime = audio.duration * dragPercent;
    ...
    async function loadOnePlaylistPage(page) {
        ...
        if (!res) {
            res = await request('/playlist/track/all/new', { listid: currentPlaylistId, page: page, pagesize: 50 }, false, 86400*30, cacheKey);
    ...
    async function openPlaylist(pl) {
        ...
        CacheManager.set(`playlist_total_${currentPlaylistId}`, currentPlaylistTotal, 86400*30, true);
