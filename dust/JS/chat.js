...
class ChatRoom {
    ...
    showNewMessageIndicator(count = 1) {
        ...
        indicator.innerHTML = count > 1 ? `<span>${count} 条新消息</span><span>↓</span>` : '<span>有新的消息</span><span>↓</span>';
        indicator.classList.add('show');
    }

    hideNewMessageIndicator() {
        document.getElementById('newMessageIndicator').classList.remove('show');
    }
    ...
    async loadMessages() {
        ...
        try {
            ...
            if (this.messages.length > 0 && newMessages.length > this.messages.length) {
                var added = newMessages.length - this.messages.length;
                if (!this.isAtBottom && !this.isUserScrolling) {
                    this.newMessageCount += added;
    ...
    async sendMessage() {
        ...
        var message = {
            ...
            userId: this.userId,
            username: this.dustUsername, // 存储带前缀的用户名
            text: text,
            timestamp: Date.now(),
    ...
    renderMessages() {
        ...
        if (oldScrollHeight > 0 && newScrollHeight > 0 && !this.isAtBottom) {
