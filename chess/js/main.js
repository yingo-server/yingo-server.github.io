...
(function() {
    ...
    // 简单音效
    function beep(f, d, t='sine', v=0.08) {
        try {
            const a = getAudioCtx();
    ...
    function render() {
        ...
        rafId = requestAnimationFrame(() => {
            ...
            if (animPiece && now - animPiece.startTime >= 180) pieceAnimDone = true;
            if (animRook && now - animRook.startTime >= 180) rookAnimDone = true;
    ...
    async function executeMove(fromIdx, move, promoType = null) {
        ...
        beep(result.captured || move.isEP ? 260 : 660, 0.04);
        lastFrom = fromIdx;
        lastTo = move.toIdx;
        animPiece = { idx: move.toIdx, startTime: performance.now(), fromScale: 0.65 };
        ...
        pollTimer = setInterval(async () => {
            ...
            try {
                ...
                if (remoteFen !== localFen) {
                    const remoteTurn = remoteFen.split(' ')[1] === 'w' ? 'white' : 'black';
                    if (data.status === 'finished' || remoteTurn !== myColor) {
                        engine.loadFEN(remoteFen);
                        selectedIdx = -1; legalMoves = [];
                        lastFrom = lastTo = -1;
                        render();
                        updateStatus();
    ...
    function doInit() {
        ...
        if (window.splashReady) {
            ...
            // 动画还未完成，设置回调等待
            const origPlaySplash = window.playSplash;
            window.tryInitApp = function() {
                if (window.splashReady) {
                    doInit();
                }
            };
