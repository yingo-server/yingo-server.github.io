(function() {
    ...
    function sfxCheck() { sfxMove(); } // 复用
    function sfxPromote() {
        sfxMove();
        setTimeout(() => sfxMove(), 80);
    }
    function sfxEnd() {
        sfxMove();
        setTimeout(() => sfxCapture(), 150);
    }
    ...
    function detectOpponentMove(oldFen, newFen) {
        ...
        let toIdx = -1, fromIdx = -1;
        for (let i = 0; i < 64; i++) {
            if (!oldBoard[i] && newBoard[i]) toIdx = i;
            if (oldBoard[i] && !newBoard[i] && i !== toIdx) fromIdx = i;
        }
        if (toIdx < 0) {
            for (let i = 0; i < 64; i++) {
        ...
        let isCastle = false, rookToIdx = -1;
        if (fromIdx >= 0 && toIdx >= 0 && oldBoard[fromIdx]?.type === 'king') {
            const fromCol = fromIdx % 8, toCol = toIdx % 8;
            if (Math.abs(toCol - fromCol) === 2) {
                isCastle = true;
                rookToIdx = toCol === 6 ? toIdx - 1 : toIdx + 1;
            }
        }
        const isCapture = oldBoard[toIdx] !== null;
