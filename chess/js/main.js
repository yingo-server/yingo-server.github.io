...
(function() {
    ...
    async function executeMove(fromIdx, move, promoType = null) {
        ...
        if (movingColor === 'white') {
            if (result.captured) {
                if (whiteMoveCount <= 3) {
                    if (window.WbyW) window.WbyW.onCaptureWithinThreeMoves();
                }
                whiteMoveCount = 0;
            } else {
                whiteMoveCount++;
            }
            lastMoveWasCapture = !!result.captured;

            const isCheck = engine.isKingInCheck(engine.turn);
            if (isCheck && lastTurnWasCheck) {
                consecutiveChecks++;
                if (consecutiveChecks >= 2) {
                    if (window.WbyW) window.WbyW.onDoubleCheck();
                    consecutiveChecks = 0;
                }
            } else {
                if (isCheck) consecutiveChecks = 1;
                else consecutiveChecks = 0;
            }
            lastTurnWasCheck = isCheck;
        }
        ...
        if (gameMode === 'online') {
            try {
                ...
                if (status.over) {
                    gameOver = true;
                    if (status.result === 'white' && window.WbyW) window.WbyW.onWhiteWin();
            ...
            if (st.over) {
                gameOver = true;
                if (st.result === 'white' && window.WbyW) window.WbyW.onWhiteWin();
        ...
        pollTimer = setInterval(async () => {
            ...
            try {
                ...
                if (data.deadMode && !deadMode) {
                    deadMode = true;
                    if (window.WbyW) window.WbyW.onDeadModeAccepted();
                    showBubble('死战模式已开启！', true);
                    notify('本局已进入【死战模式】，无法求和或认输！', 'warn');
                }

                if (data.deadModeMessage && !gameOver) {
                    if (window.WbyW) window.WbyW.onDeadModeMessage();
                ...
                if (!gameOver && currentRoomStatus === 'playing' && engine.turn !== myColor && myColor === 'white') {
                    const now = Date.now();
                    const lastMove = new Date(data.lastMoveTime).getTime();
                    if (!isNaN(lastMove) && now - lastMove > 10000 && !opponentTimeoutTriggered) {
                        opponentTimeoutTriggered = true;
                        if (window.WbyW) window.WbyW.onOpponentTimeout();
                ...
                if (remoteFen !== localFen && remoteFen !== lastPolledFen) {
                    ...
                    if (data.status === 'finished') {
                        gameOver = true;
                        if (data.result === 'white' && window.WbyW) window.WbyW.onWhiteWin();
    ...
    function startLocalGame() {
        ...
        setLocalAvatars();  // 显示本地模式头像
        lastPolledFen = null;
        deadMode = false;
        deadModeRequestSent = false;
        whiteMoveCount = 0;
        consecutiveChecks = 0;
        lastTurnWasCheck = false;
