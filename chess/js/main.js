...
(function() {
    ...
    async function requestAIMove() {
        ...
        const uciMove = await window.AiEngine.getBestMove(fen, 5000); // 5秒
        aiThinking = false;
        if (!uciMove) {
        ...
        if (window.AiEngine) {
            ...
            window.AiEngine.setLogCallback(function(msg) {
                ...
                if (logDiv) {
                    ...
                    const newLine = `[${time}] ${msg}`;
                    logDiv.innerHTML = newLine + '<br>' + logDiv.innerHTML;
                    const lines = logDiv.innerHTML.split('<br>');
                    if (lines.length > 15) {
                        logDiv.innerHTML = lines.slice(0, 15).join('<br>');
                    }
                    const logArea = document.getElementById('engineLogArea');
