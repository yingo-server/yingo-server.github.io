...
(function() {
    ...
    // AI 模式死战模式请求状态
    let aiDeadModeRequestPending = false;
    let aiDeadModeRequestCallback = null;
    ...
    async function aiOfferDraw() {
        ...
        if (gameMode !== 'ai' || gameOver) return;
        const accept = !window.AiEngine.isEvenSecond(); // 奇秒接受
        if (accept) {
    ...
    async function aiResign() {
        ...
        updateStatus();
        sfxEnd();
        if (window.WbyW) window.WbyW.onWhiteWin(false);
        notify('您已认输', 'err');
    }
