...
(function() {
    ...
    let pendingDrawOffer = false;      // 我是否发出了求和请求
    let pendingDrawResponse = false;   // 是否正在处理对方的求和弹窗（避免重复）
    let myDrawOfferSent = false;       // 辅助检测拒绝
    ...
    function startPolling() {
        ...
        pollTimer = setInterval(async () => {
            ...
            try {
                ...
                // ----- 处理对方求和请求（弹窗）-----
                if (data.drawOffer && data.drawOffer !== myColor && !gameOver && !pendingDrawResponse) {
                ...
                // ----- 检测我发出的求和是否被对方拒绝 -----
                // 如果我正在等待回复（pendingDrawOffer == true），但房间里的 drawOffer 不再是 myColor（可能是 null 或被对方改成了其他颜色），说明对方已经拒绝或超时清除了。
