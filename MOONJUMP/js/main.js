// main.js - 修复版：解决进入地图报错、存档逻辑、移动端手势冲突等问题
(function () {
    // ==================== 全局配置 ====================
    const CONFIG = {
        desert: {
    ...
    class Game {
        ...
        // 修复2：将 setupSkipButton 变为正确的类方法
        setupSkipButton() {
            if (!skipBtn) return;
        ...
        async transitionToNextLevel() {
            ...
            // 修复3：存档逻辑改为“下一个可玩的关卡”，避免玩家被卡在已通关关卡
            ...
            this.currentLevel = nextLevel; // 直接更新到下一关
    ...
    function renderLevelItems(listEl, levels, mode) {
        levels.forEach(level => {
            ...
            // 注意：这里的“已完成”标记应以已存储的“下一个可玩关卡”为准。
            // 因为通关后我们保存的是下一关的编号，所以这里用“已通过关卡”来标记。
