// main.js - 完整版：平滑回正 + FPS 鼠标 + 严重错误修复 + 背景音乐
(function () {
    // ==================== 全局配置 ====================
    const CONFIG = {
        desert: {
    ...
    class Game {
        ...
        async startLevel(levelNum, mode) {
            ...
            const endQuat = new THREE.Quaternion().setFromRotationMatrix(
                new THREE.Matrix4().lookAt(startPos, goalLookAt, new THREE.Vector3(0, 1, 0))
            );
        ...
        processCameraFlight(dt, now) {
            ...
            if (cf.phase === 'toGoal') {
                ...
                if (now - cf.phaseStart >= 1.0) {
                    ...
                    cf.startQuat = this.camera.quaternion.clone();
                }
                ...
                if (cf.startQuat && cf.endQuat) {
                    THREE.Quaternion.slerp(cf.startQuat, cf.endQuat, this.camera.quaternion, t);
                } else {
                    this.camera.lookAt(cf.goalLookAt);
                }
                if (t >= 1.0) {
                    this.camera.position.copy(cf.startPos);
                    if (cf.endQuat) this.camera.quaternion.copy(cf.endQuat);
                    else this.camera.lookAt(cf.goalLookAt);
                    if (this.player) {
                        const euler = new THREE.Euler().setFromQuaternion(this.camera.quaternion, 'YXZ');
                        this.player.yaw = euler.y;
                        this.player.pitch = euler.x;
                        this.player.camera.position.copy(cf.startPos);
                        this.player.camera.quaternion.copy(this.camera.quaternion);
                    }
                    this.cameraFlight = null;
                    return true;
                }
