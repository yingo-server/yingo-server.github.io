// main.js - 最终完整版：2.0 地图标准 + 双语 tooltip + FPS 鼠标 + 进入关卡播放音乐 + 增强碰撞与严重错误修复
(function () {
    // ==================== 全局配置 ====================
    const CONFIG = {
        desert: {
    ...
    async function loadLevelList(mode) {
        ...
        for (let i = start; i <= end; i++) {
            try {
                ...
                catch (parseErr) { console.warn(`关卡 ${i} JSON 解析失败:`, parseErr.message); break; }
    ...
    async function boot() {
        ...
        prepareBackgroundMusic();   // 只创建对象，不播放
