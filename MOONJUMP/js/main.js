// main.js - 完整修复版：资源释放/异常处理/状态锁定/UI优化/关卡加载超时/动态平台位置修正
(function () {
    ...
    skipBtn.style.pointerEvents = 'none'; // 转场期间禁用点击
    ...
    class Game {
        ...
        async loadLevel(levelNum) {
            ...
            try {
                ...
                // 验证必需字段
                if (!data || !data.startPosition || isNaN(data.startPosition.x)) {
                    throw new Error('无效的关卡数据：缺少 startPosition');
                }
                ...
                try {
                    ...
                    console.warn('无法保存进度', storageErr);
                }
        ...
        buildFallbackLevel() {
            ...
            ground.userData.isGround = true; ground.userData.isGoal = false;
            ground.userData.tooltip = ''; // 无提示，避免困惑
            ...
            edgeLine.position.copy(ground.position);
        ...
        setupSkipButton() {
            skipBtn.addEventListener('click', () => {
                if (!this.transitioning) this.transitionToNextLevel();
            });
    ...
    async function startGame() {
        ...
        try {
            ...
            console.error('首关加载异常:', e);
            // 确保游戏仍可进入（后备已在 loadLevel 中处理）
        }
        // 首关转场
        transitionOverlay.style.transition = 'opacity 0.8s';
        transitionOverlay.style.opacity = '1';
