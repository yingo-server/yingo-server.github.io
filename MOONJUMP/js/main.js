...
(function () {
    ...
    class Game {
        constructor(canvas) {
            ...
            this.scene.add(this.sunLight.target); this.scene.add(this.sunLight);
            ...
            this.goalStandTimer = 0;            // 站在终点的时间
            this.goalRequiredTime = 0.8;        // 静默等待时长
            this.goalReached = false;
            this.currentLevel = 1;
            this.transitioning = true;
            this.shownTooltips = new Set();
            this.currentLevelName = '';
            this.goalPosition = null;          // 终点平台位置
            this.cameraFlight = null;          // 摄像机飞行状态
        ...
        clearScene() {
            const toRemove = [];
            const keepSet = new Set([this.ambient, this.sunLight, this.moonSprite, this.moonBrightLayer, this.glowSprite, this.stars, this.sunLight.target]);
            this.scene.children.forEach(c => { if (!keepSet.has(c)) toRemove.push(c); });
        ...
        async loadLevel(levelNum) {
            ...
            try {
                ...
                try { localStorage.setItem('blockRunLevel', levelNum); } catch (e) {}
        ...
        buildFallbackLevel() {
            ...
            ground.userData.isGround = true; ground.userData.isGoal = false; ground.userData.tooltip = '';
            this.scene.add(ground); this.worldMeshes.push(ground);
            const edge = new THREE.LineSegments(new THREE.EdgesGeometry(geo), new THREE.LineBasicMaterial({ color: 0x888888 }));
            edge.position.copy(ground.position); this.scene.add(edge);
        }
        ...
        buildLevel(data) {
            (data.platforms || []).forEach(p => {
                ...
                if (p.isGoal) {
                    this.goalPlatformMesh = mesh;
                    this.goalPosition = mesh.position.clone();
                }
                mesh.userData.tooltip = p.tooltip || '';
                this.scene.add(mesh); this.worldMeshes.push(mesh);
                const edgeLine = new THREE.LineSegments(new THREE.EdgesGeometry(geo), new THREE.LineBasicMaterial({ color: 0x88aaff }));
            ...
            if (cf.phase === 'toGoal') {
                ...
                if (t >= 1.0) {
                    ...
                    if (this.player) {
                        ...
                        dir.subVectors(cf.goalLookAt, cf.startPos).normalize();
        ...
        async transitionToNextLevel() {
            ...
            // 开始摄像机飞行
            const startPos = this.player.startPosition.clone();
            ...
            const toGoalPos = goalPos.clone().add(new THREE.Vector3(0, 3, 0)); // 终点上方3单位
            this.cameraFlight = {
                phase: 'toGoal',
            ...
            // 等待飞行完成（在 animate 中处理）
            await new Promise(resolve => {
                this._flightResolve = resolve;
            });
            ...
            if (skipBtn) { skipBtn.style.pointerEvents = 'auto'; skipBtn.style.opacity = '0.7'; }
        ...
        animate() {
            ...
            if (this.cameraFlight) {
                ...
                if (finished && this._flightResolve) {
                    this._flightResolve();
    ...
    preloadMoonTexture(() => {});
    setTimeout(() => { if (!game) startGame(); }, 4500);
    ...
    async function startGame() {
        ...
        // 首关也执行飞行序列
        const startPos = game.player.startPosition.clone();
        let goalPos = game.goalPosition ? game.goalPosition.clone() : startPos.clone().add(new THREE.Vector3(0, 0, 10));
        const goalLookAt = goalPos.clone();
        const toGoalPos = goalPos.clone().add(new THREE.Vector3(0, 3, 0));
        game.cameraFlight = {
            phase: 'toGoal',
            phaseStart: performance.now() / 1000,
            from: startPos.clone(),
        ...
        await new Promise(resolve => { game._flightResolve = resolve; });
        animateDivider();
        await new Promise(r => setTimeout(r, 1300));
