// main.js - 3.0 版本，支持保存点，修复闪烁重复触发问题
(function () {
    // ==================== 全局配置 ====================
    const CONFIG = {
        environment: {
        ...
        checkpoint: {
            blinkInterval: 0.2,      // 闪烁间隔（秒）
            blinkCount: 6,           // 总共闪烁6次（3次亮暗循环）
        }
    };
    ...
    class Player {
        constructor(camera, scene, startPos) {
            ...
            this.checkpoint = startPos.clone();   // 复活点
    ...
    class Game {
        constructor(canvas) {
            ...
            this.checkpointMeshes = [];     // 保存点平台列表
            this.lastCheckpointMesh = null; // 用于避免重复闪烁
            this.player = null;
        ...
        blinkCheckpoint(mesh) {
            if (!mesh || !mesh.material) return;
            if (mesh.userData.isBlinking) return; // 防止重复闪烁
            mesh.userData.isBlinking = true;
            const originalColor = mesh.material.color.clone();
            let step = 0;
            const interval = setInterval(() => {
                if (step >= CONFIG.checkpoint.blinkCount) {
                ...
                step++;
            }, CONFIG.checkpoint.blinkInterval * 1000);
        }
        ...
        checkAndUpdateCheckpoint() {
            ...
            if (mesh.userData.isCheckpoint) {
                ...
                // 仅当首次踏上或切换到不同保存点时触发闪烁
                if (this.lastCheckpointMesh !== mesh) {
                    this.blinkCheckpoint(mesh);
                    this.lastCheckpointMesh = mesh;
                }
            } else {
                // 离开保存点平台时不清空 lastCheckpointMesh，以便下次踏上同一平台不再闪烁
                // 但为了逻辑清晰，如果玩家离开保存点，可以重置 lastCheckpointMesh，
                // 这样再次踏上同一个保存点时会重新闪烁。根据设计，离开后再来应该再闪烁一次，更合理。
                if (this.lastCheckpointMesh === mesh) {
                    this.lastCheckpointMesh = null;
                }
            }
        ...
        buildLevel(data) {
            ...
            (data.platforms || []).forEach(p => {
                ...
                let color = CONFIG.environment.platformColor;
                if (p.type === 1) color = 0x44aa44;
                else if (p.type === 2) color = 0x44aaff;
                else if (p.type === 3) color = CONFIG.environment.goalPlatformColor;
                ...
                mesh.userData.isBlinking = false; // 闪烁标志
                this.scene.add(mesh); this.worldMeshes.push(mesh);
                if (p.type === 2) this.checkpointMeshes.push(mesh);

                const edgeLine = new THREE.LineSegments(new THREE.EdgesGeometry(geo), new THREE.LineBasicMaterial({ color: p.type === 3 ? '#aaaaaa' : '#444444' }));
