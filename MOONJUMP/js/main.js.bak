// main.js - 性能优化版（沙漠贴图重复减半 + 地面跟随 + 动态关卡加载）
(function () {
    // ==================== 全局配置 ====================
    const CONFIG = {
        desert: {
            ...
            groundY: -8,
            groundSize: 400,
            repeatCount: 20,             // 原40，减半后纹理覆盖面积增大4倍，采样性能提升
        ...
        groundFollow: {
            threshold: 80,      // 玩家距地面中心超过此值触发移动
            step: 100,          // 网格移动步长（单位）
        },
        levelLoad: {
            concurrent: 5,      // 每次并发检测5个关卡
            maxFails: 3,        // 连续失败3次停止检测
        }
    };
    ...
    class Player {
        constructor(camera, scene, startPos) {
            ...
            this.inheritedVelocity = new THREE.Vector3(); // 来自移动平台的额外速度
            this.yaw = 0;
            this.pitch = 0;
        ...
        update(dt, input, worldMeshes, wallMeshes) {
            ...
            if (this.isGrounded && this.lastGroundMesh) {
                ...
                if (delta.length() > 0.001) {
                    ...
                    // 计算平台瞬时速度（单位/秒）
                    const platformVel = delta.clone().divideScalar(dt);
                    this.inheritedVelocity.copy(platformVel);
                }
                this.lastGroundPos.copy(currentGroundPos);
            } else {
                // 离开地面后，继承动量逐渐衰减
                this.inheritedVelocity.multiplyScalar(0.95);
                if (this.inheritedVelocity.length() < 0.01) this.inheritedVelocity.set(0, 0, 0);
            }
            ...
            // 应用继承动量（仅水平方向）
            this.position.x += this.inheritedVelocity.x * dt;
            this.position.z += this.inheritedVelocity.z * dt;
    ...
    class Game {
        ...
        createDesertGround() {
            ...
            if (this.groundGroup) this.scene.remove(this.groundGroup);
            ...
            this.updateGroundPosition(new THREE.Vector3(0, 0, 0));
        }

        updateGroundPosition(playerPos) {
            if (!this.groundGroup) return;
            const step = CONFIG.groundFollow.step;
            const centerX = Math.round(playerPos.x / step) * step;
            const centerZ = Math.round(playerPos.z / step) * step;
            this.groundGroup.position.set(centerX, CONFIG.desert.groundY, centerZ);
        }
        ...
        updateSkyAndLighting(playerPos) {
            ...
            // 地面跟随
            this.updateGroundPosition(playerPos);
        }
        ...
        buildFallbackLevel() {
            ...
            mesh.userData.isGround = true; mesh.userData.isGoal = false;
            mesh.userData.tooltip = ''; mesh.userData.tooltip_en = ''; mesh.userData.id = '';
        ...
        buildLevel(data) {
            ...
            (data.platforms || []).forEach(p => {
                ...
                mesh.userData.tooltip_en = p.isGoal ? '' : (p.tooltip_en || '');
    ...
    async function loadLevelList(mode, startFrom = 1) {
        ...
        const CONCURRENT = CONFIG.levelLoad.concurrent;
        const MAX_FAILS = CONFIG.levelLoad.maxFails;

        while (loadingActive && consecutiveFails < MAX_FAILS) {
            const batch = [];
            ...
            if (!anySuccess && consecutiveFails >= MAX_FAILS) break;
            nextId += CONCURRENT;
        }
        const loadingEl = document.getElementById(mode === 'trial' ? 'trial-loading' : 'challenge-loading');
        if (loadingEl) loadingEl.textContent = consecutiveFails >= MAX_FAILS ? '没有更多关卡' : '向下滑动加载更多';
        isLoadingMore = false;
    }
