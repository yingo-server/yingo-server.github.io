...
const PERFORMANCE_CONFIG = {
    ...
    debounceScroll: 100, // 滚动防抖延迟
    optimizeForLowEnd: false // 自动检测低端设备
};

// 检测低性能设备
function detectLowPerformance() {
    const isLowEnd = (
        navigator.hardwareConcurrency <= 2 || // CPU核心数少
        /(Android|webOS|iPhone|iPad|iPod|BlackBerry|Windows Phone)/i.test(navigator.userAgent) // 移动设备
    );
    
    PERFORMANCE_CONFIG.optimizeForLowEnd = isLowEnd;
    console.log(`设备性能检测: ${isLowEnd ? '低性能设备' : '标准设备'}`);
    
    if (isLowEnd) {
        // 低性能设备优化
        API_CONFIG.preloadRange = 3; // 减少预加载范围
        API_CONFIG.initialLoadCount = 5; // 减少初始加载数量
        API_CONFIG.requestDelay = 1500; // 增加请求延迟
...
function addOptimizedEventListener(element, event, handler, options = {}) {
    if (PERFORMANCE_CONFIG.usePassiveListeners && 
