 ...
 * 功能：从 GitHub API 获取文件列表，播放路径使用本地 /mfile/，从 MP3 标签提取封面图片
 * 开关：FOOT_ENABLED (true: 启用; false: 完全静默)
 * 
 * 【修改说明】
 * - 播放 URL 改为本地路径：/mfile/文件名
 * - 封面从 MP3 的 ID3 标签中提取（使用 jsmediatags 库）
 * - 修复手机端音量滑块超出屏幕的问题
 * - 移除镜像代理和 PNG 封面依赖
 */
...
(function() {
    ...
    // ========== 本地播放路径配置 ==========
    const LOCAL_MUSIC_BASE = "/mfile/";  // 音乐文件所在目录（相对于网站根目录）
    ...
    let coverCache = new Map();  // 缓存封面图片 URL
    ...
    // ========== 从 MP3 提取封面图片 ==========
    async function extractCoverFromMp3(mp3Url) {
        // 检查缓存
        if (coverCache.has(mp3Url)) {
            return coverCache.get(mp3Url);
        }
            ...
            window.jsmediatags.read(mp3Url, {
                onSuccess: (tag) => {
                    ...
                    if (picture && picture.data) {
                        ...
                        coverCache.set(mp3Url, imageUrl);
                        resolve(imageUrl);
                    } else {
                        resolve(null);
                    }
                ...
                onError: (error) => {
                    console.warn('提取封面失败:', mp3Url, error);
                    resolve(null);
                }
            });
        });
    ...
    // ========== 更新封面显示 ==========
    async function updateCoverDisplay(mp3Url) {
        if (!coverDiv) return;

        try {
            const imageUrl = await extractCoverFromMp3(mp3Url);
            if (imageUrl) {
                coverDiv.innerHTML = '';
            ...
            console.warn('更新封面失败:', error);
            coverDiv.innerHTML = '<span class="material-symbols-outlined">music_note</span>';
        }
    }

    // ========== GitHub API 请求（获取文件列表）==========
    ...
    function createPlayer() {
        ...
        coverDiv = document.getElementById('player-cover');
    ...
    async function loadTrack(index) {
        ...
        // 更新封面（从 MP3 标签提取）
        await updateCoverDisplay(track.mp3Url);
    ...
    async function init() {
        ...
        try {
            ...
            console.log('jsmediatags 加载成功');
        } catch (error) {
            console.warn('jsmediatags 加载失败，将无法显示封面:', error);
        }
