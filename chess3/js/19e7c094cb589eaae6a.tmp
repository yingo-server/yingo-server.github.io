...
(function() {
    ...
    async function executeMove(fromIdx, move, promoType = null) {
        ...
        if (movingColor === 'white') {
            // 吃子语音 + 气泡（白方自己：显示在己方头像，isOpponent = false）
            if (result.captured) {
                if (window.WbyW) window.WbyW.onCaptureWithinThreeMoves(false);
            }
            // 将军语音 + 气泡
            if (engine.isKingInCheck(engine.turn)) {
                if (window.WbyW) window.WbyW.onDoubleCheck(false);
            }
        }
        ...
        pollTimer = setInterval(async () => {
            ...
            try {
                ...
                if (remoteFen !== localFen && remoteFen !== lastPolledFen) {
                    ...
                    // 关键：黑方端收到白方移动时，触发白方语音和气泡（显示在对手头像旁，即 isOpponent = true）
                    if (moveInfo.isCapture && window.WbyW) {
                        window.WbyW.onCaptureWithinThreeMoves(true);
                    }
                    if (engine.isKingInCheck(engine.turn) && window.WbyW) {
                        window.WbyW.onDoubleCheck(true);
                    }
                    ...
                    if (data.status === 'finished') {
                        gameOver = true;
                        if (data.result === 'white' && window.WbyW) window.WbyW.onWhiteWin(false);
