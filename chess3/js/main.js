...
(function() {
    ...
    // 白方语音历史记录（本地同步）
    let whiteConsecutiveMoves = 0;      // 白方连续走子步数（黑方走棋后清零）
    let whiteConsecutiveChecks = 0;     // 白方连续将军次数
    let opponentTimeoutTriggered = false; // 超时语音防重复标志
    ...
    // 统一评估白方语音条件（双方调用，isOpponentBubble 区分气泡位置）
    function evaluateWhiteVoice(movingColor, isCapture, isCheckAfterMove, isOpponentBubble) {
        ...
        if (isCapture) {
            ...
            whiteConsecutiveMoves = 0; // 吃子后重置计数
        }
        // 连将两次
        if (isCheckAfterMove) {
            ...
            try {
                ...
                if (!gameOver && currentRoomStatus === 'playing') {
                    ...
                    if (!isNaN(lastMove) && now - lastMove > 10000) {
                        ...
                        // 黑方检测（自己是黑方，且轮到黑方自己，说明自己超时未走棋）
                        if (myColor === 'black' && engine.turn === myColor && !opponentTimeoutTriggered) {
                            opponentTimeoutTriggered = true;
                            if (window.WbyW) window.WbyW.onOpponentTimeout(true); // 黑方视角，气泡显示在白方头像
                        }
                ...
                if (remoteFen !== localFen && remoteFen !== lastPolledFen) {
                    ...
                    // 同步白方战术语音（对手移动时，气泡显示在对手头像）
                    const movedPieceAfter = engine.board[lastTo];
                    const movingColor = movedPieceAfter ? movedPieceAfter.color : null;
